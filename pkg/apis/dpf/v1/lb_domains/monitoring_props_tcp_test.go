...
var _ = Describe("MonitoringPorpsTCP", func() {
	var (
		bs         []byte
		c          lb_domains.MonitoringPorpsTCP
		err        error
		s1, s2, s3 lb_domains.MonitoringPorpsTCP
	)
	BeforeEach(func() {
		s1 = lb_domains.MonitoringPorpsTCP{
		...
		s2 = lb_domains.MonitoringPorpsTCP{
			MonitoringPorpsCommon: lb_domains.MonitoringPorpsCommon{
		...
		s3 = lb_domains.MonitoringPorpsTCP{
			Port: 53,
		}
	})
	Context("Read", func() {
